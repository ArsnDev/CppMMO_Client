...
namespace SimpleMMO.Network
{
    public class GameServerClient : MonoBehaviour
    {
        ...
        {
            Debug.LogWarning($"SendPacket called with {packetData.Length} bytes");
            
            if (!_isConnected || _stream == null)
            {
                Debug.LogError("Cannot send packet, not connected to the server.");
                return;
            }
            ...
            try
            {
                ...
                {
                    Debug.LogError("Connection lost during packet send");
                    Disconnect();
                    return;
                }
                
                Debug.LogWarning("About to write packet size header");
                byte[] sizeBytes = BitConverter.GetBytes(packetData.Length);
                _stream.Write(sizeBytes, 0, 4);
                _stream.Flush(); // 헤더 먼저 플러시
                
                Debug.LogWarning("About to write packet data");
                _stream.Write(packetData, 0, packetData.Length);
                _stream.Flush(); // 데이터 플러시
                
                Debug.Log($"✅ Successfully sent packet: {packetData.Length} bytes");
            }
            catch (Exception ex)
            {
        ...
        private void ProcessPacket(byte[] packetData)
        {
            ...
            switch(packetId)
            {
                ...
                case PacketId.S_PlayerJoined:
                    var playerJoinedData = packet.DataAsS_PlayerJoined();
                    Debug.Log($"🟢 [S_PlayerJoined] Received - PlayerId: {playerJoinedData.PlayerInfo?.PlayerId}, Name: {playerJoinedData.PlayerInfo?.Name}");
